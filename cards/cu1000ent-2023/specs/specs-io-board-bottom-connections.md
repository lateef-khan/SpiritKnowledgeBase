---
id: cu1000ent-2023-specs-io-board-bottom-connections
title: 'Nine sockets on the console I/O board: USB, ground, console power, internet,
  4-pin C-safe, HDMI, TV, 20-pin keyboard and 8-pin system cable'
kind: spec
question: What plugs into the console I/O board on a Spirit cu1000ent-2023 upright
  bike?
asked_as:
- what cables go into the back of the cu1000 console
- cu1000ent i/o board connections
- how many pins is the keyboard cable on the cu1000
- c-safe connector cu1000ent
keywords:
- i/o board
- console
- usb connect
- ground
- console power
- internet
- c-safe 4 pins
- hdmi
- tv connect
- key board 20 pins
facets:
  brand:
  - spirit
  product_line: bike
  model: cu1000ent-2023
  applies_to:
  - cu1000ent-2023
  section: specs
  code: '*'
  model_number:
  - '210354'
authority: 3
not_to_be_confused_with:
- cu900ent-io-board-connections
see_also:
- cu1000ent-2023-specs-driver-board-cs56012-connections
- cu1000ent-2023-specs-circuit-diagram
source:
  ref: spirit-bike-cu1000ent-2023-service-manual
  locator: Section 6.1 I/O Board Bottom, PDF p. 10 (printed 10), text.md lines 171-202
  extracted_at: '2026-09-11'
---

Nine call-outs on the underside of the console:

- **USB CONNECT**
- **GROUND**
- **Console Power CONNECT**
- **Internet CONNECT**
- **C-safe CONNECT (4 PINS)**
- **HDMI CONTACT**
- **TV CONNECT**
- **Key Board CONNECT (20 PINS)**
- **SYSTEM CABLE CONNECT (8 PINS)**

**Two differences from the CU900ENT I/O board** (`cu900ent-io-board-connections`): the C-safe
socket is called out as **4 pins** here, and the two hand-pulse wires of the CU900ENT are replaced
by a single **20-pin keyboard** connector - on this bike the hand-pulse leads plug into the keypad,
not the console (the keyboard replacement procedure disconnects "20pin and Handpulse Wire on the
keyboard"). The 8-pin system cable is the same count on both.

