---
id: cvc800-quick-start
title: Quick start - starting, changing resistance, pausing and stopping
kind: procedure
question: How do I start a workout on a Spirit CVC800 climber?
asked_as:
- how do i start my spirit climber
- how do i change the resistance on the climber
- how do i pause the climber
keywords:
- quick start
- start key
- stop key
- pause
- resume
- up down keys
- workload
- resistance
- time counts up
facets:
  brand:
  - spirit
  product_line: climber
  model: cvc800
  applies_to:
  - cvc800
  section: console
  code: '*'
  model_number:
  - '800440'
authority: 3
not_to_be_confused_with: []
see_also:
- cvc800-workout-mode-buttons
- cvc800-idle-mode-buttons
- cvc800-program-buttons
- cvc800-console-fls-and-fpm-in-the-message-window
source:
  ref: spirit-climber-cvc800-service-manual
  locator: Section 4 Product Operation, Operation, QUICK START, p. 13 (printed 13)
  extracted_at: '2026-09-08'
---

1. Wait for the console to power up.
2. Press **START**. This is QUICK START mode.

In QUICK START the **TIME counts up from zero** and the workload/resistance is adjusted manually
with the **UP / DOWN** keys.

- Press **STOP once** to **pause**. Press **START** to resume.
- Press **STOP twice** to end the workout.

Pressing STOP a third time returns the console to idle mode, and holding STOP for 3 seconds in idle
mode resets the console.

**The 2021 owner's manual prints this procedure word for word** on its printed p. 16, under
QUICK START, so both books agree: Start begins, Time counts up from zero, Up/Down set the workload,
Stop once pauses, Start resumes, Stop twice ends.

**The owner's manual adds one line the service manual has not**, immediately below the procedure:
`NOTE: FLS (Total Floors Climbed), FPM (Floors Per Minute)` -
`cvc800-console-fls-and-fpm-in-the-message-window`.
