---
id: f89-2023-model-overview
title: What this service manual covers
kind: fact
question: Which treadmill does the Sole f89-2023 service manual cover and what is
  its factory model code?
asked_as:
- what model is the sole f89 2023
- what is st378-yt078
- which service manual do i use for a 2023 treadmill
keywords:
- st378-yt078
- model code
- service manual
- '2023'
- treadmill
- console size
- touchscreen
facets:
  brand:
  - sole
  product_line: treadmill
  model: f89-2023
  applies_to:
  - f89-2023
  section: specs
  code: '*'
  model_number:
  - '589853'
authority: 3
not_to_be_confused_with: []
see_also:
- f80-2023-electrical-parts-overview
- f80-2023-drive-motor-spec
- f80-2023-incline-motor-spec
- f89-model-numbers
source:
  ref: sole-tm-f89-2023-service-manual
  locator: Cover, page 1; Section 6.1 Display Board wire Connections, page 25
  extracted_at: '2026-09-04'
---

The cover reads **F89(2023)**, the factory code **ST378-YT078**, and calls the machine
a **ENT Treadmill**.

The console is key controls and a 21.5" TFT touch panel display. The display board runs **Android 10**.

The machine has a console, a main controller carrying the DC power supply, the incline driver and the DC motor
driver, a DC drive motor and an AC incline motor. It also has a **rear incline motor** on its own rear incline control board, which is what gives this machine decline.

**The manual never prints a six digit SKU.** It identifies the machine only by the name on the cover and the
factory code above. The model number is on the F89 model card, `f89-model-numbers`.
