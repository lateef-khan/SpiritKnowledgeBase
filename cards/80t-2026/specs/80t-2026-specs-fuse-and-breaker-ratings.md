---
id: 80t-2026-specs-fuse-and-breaker-ratings
title: The circuit breaker is rated AC 250 V 10 A and the inverter fuse 125 Vac 1 A
kind: spec
question: What are the circuit breaker and inverter fuse ratings on a Spirit 80t-2026 treadmill?
asked_as:
- what fuse does this treadmill take
- what size breaker is on the treadmill
- the fuse on the inverter keeps blowing what is it
- breaker rating on the 8.0t
keywords:
- fuse
- circuit breaker
- breaker rating
- inverter fuse
- breaking capacity
- thermal trip
- amp
- vac
- replacement fuse
- cr fuse size
facets:
  brand:
  - spirit
  product_line: treadmill
  model: 80t-2026
  applies_to:
  - 80t-2026
  section: specs
  code: '*'
  model_number:
  - '780885'
authority: 3
not_to_be_confused_with: []
see_also:
- 80t-2026-specs-dimensions-and-product-weight
- spirit-2026t-specs-drive-motor-rating
source:
  ref: spirit-treadmill-80t-2026-owners-manual
  locator: SPECIFICATIONS p. 65, Fuse rating and Fuse on inverter blocks
  extracted_at: '2026-09-09'
---

Two separate devices, with separate ratings. Do not order one against the other's
numbers.

**Fuse rating - the circuit breaker.**

| Property | Value |
|---|---|
| Circuit breaker | AC 250V, 10A |
| Mode of tripping | thermal |
| Operating speed | 0.25 ~ 3.0 sec |
| Breaking capacity | 500 A, 250 Vac |

**Fuse on inverter.**

| Property | Value |
|---|---|
| Voltage | 125 Vac |
| Current | 1 A |
| Operating speed | 0.4 ~ 1.0 sec |
| Breaking capacity | 50 A, 125 Vac |
| Size | 2.65 x 7.1 mm |

The manual gives no part number for either, and no replacement procedure on this
page. The 7.0T and 4.0T manuals print no fuse or breaker rating at all.
