---
id: xterra-warranty-owner-pays-freight-and-parts-shipping-no-trip-charge-printed
title: What the owner pays during a warranty repair - freight and parts shipping,
  with no trip charge printed
kind: policy
question: What costs do I have to pay for warranty service on an Xterra SB150, SB250
  or SB2.5r recumbent bike, EU100 or EU150 hybrid elliptical, or FS150, FS1.5, FS2.5,
  FS3.0 or FS3.5 elliptical?
asked_as:
- do i pay shipping for warranty parts on my xterra elliptical
- is there a trip charge for the fs2.5
- who pays freight to the service center
- sb150 warranty service costs
keywords:
- owner costs
- freight charges
- shipping and handling
- replacement parts
- no trip charge
- service center
- preauthorized
- return authorization
- warranty repair
- in-home service
facets:
  brand:
  - xterra
  product_line: '*'
  model: '*'
  applies_to:
  - eu100-2018
  - eu150-2024
  - fs15-2019
  - fs150-2016
  - fs25-2020
  - fs30-2018
  - fs35-2020
  - sb150-2018
  - sb250-2024
  - sb25r-2020
  section: warranty
  code: '*'
authority: 3
not_to_be_confused_with:
- xterra-treadmill-warranty-owner-pays-freight-parts-shipping-and-trip-charges
see_also:
- xterra-treadmill-warranty-service-steps-dealer-service-organization-then-the-service-department
- xterra-treadmill-warranty-owner-pays-freight-parts-shipping-and-trip-charges
- xterra-warranty-exclusions-eight-items-no-service-call-reimbursement-item
- xterra-warranty-periods-frame-1-year-parts-1-year-labor-none
- xterra-warranty-periods-frame-5-years-parts-1-year-no-labor-column
- xterra-treadmill-warranty-consumer-responsibilities-five-items-with-a-registration-card
source:
  ref: xterra-elliptical-fs30-2018-owners-manual
  locator: SB150 (sb150-2018) PDF p. 26 (printed 24), lines 798-866, effective August
    21, 2018; SB250 (sb250-2024) PDF p. 27 (printed 25), lines 893-961, effective
    June 21, 2019; SB2.5r (sb25r-2020) PDF p. 19 (printed 17), lines 638-706, effective
    July 06, 2020; EU100 (eu100-2018) PDF p. 19 (printed 16), lines 528-596, effective
    August 21, 2018; EU150 (eu150-2024) PDF p. 27 (printed 24), lines 736-804, effective
    April 26, 2024; FS150 (fs150-2016) PDF p. 26 (scan of printed 24), OCR lines 1090-1181,
    effective June 6, 2016; FS1.5 (fs15-2019) PDF p. 19 (printed 17), lines 622-690,
    effective February 15, 2019; FS2.5 (fs25-2020) PDF p. 19 (printed 17), lines 681-749,
    effective July 06, 2020; FS3.0 (fs30-2018) PDF p. 19 (printed 17), lines 665-733,
    effective August 21, 2018; FS3.5 (fs35-2020) PDF p. 19 (printed 17), lines 673-741,
    effective July 06, 2020
  extracted_at: '2026-09-11'
---

Warranty step 4 in these ten books puts two costs on the customer while the machine is under warranty:

- **Freight to and from a service center**, if the product is shipped to one.
- **Shipping and handling on replacement parts** shipped while the product is under warranty.

The sentence before them sets the frame: XTERRA Fitness's obligation is *limited to repairing or replacing, at XTERRA Fitness's option, the product through one of our authorized service centers*, and **all repairs must be preauthorized**. The step then runs straight on into the packaging and return-authorization rule: *The owner is responsible for adequate packaging upon return to XTERRA Fitness. XTERRA Fitness is not responsible for damages in shipping. Make all freight damage claims with the appropriate freight carrier. DO NOT SHIP ANY UNIT TO OUR FACTORY WITHOUT A RETURN AUTHORIZATION NUMBER. All units arriving without a return authorization number will be refused.* - so these books have **five** service steps, not six, with the P.O. Box address as step 5 (`xterra-treadmill-warranty-service-steps-dealer-service-organization-then-the-service-department`).

**No trip charge is printed.** The other Xterra books add *For in-home service, the customer will be responsible for a trip charge. There will be an additional trip charge if the customer is located over 100 miles from the nearest service center* (`xterra-treadmill-warranty-owner-pays-freight-parts-shipping-and-trip-charges`); these ten do not. The same ten drop the service-call reimbursement item from their exclusions (`xterra-warranty-exclusions-eight-items-no-service-call-reimbursement-item`). None of that makes in-home labor free: their tables print *Labor None* (`xterra-warranty-periods-frame-1-year-parts-1-year-labor-none`) or no labor column at all (`xterra-warranty-periods-frame-5-years-parts-1-year-no-labor-column`), so no technician visit is covered and the question of a trip charge on a covered visit does not arise. What the customer gets under warranty is the part, shipped at the customer's expense.

**Ten books print this shortened step 4**: SB150, SB250, SB2.5r, EU100, EU150, FS150, FS1.5, FS2.5, FS3.0, FS3.5. Making the machine accessible for servicing stays consumer responsibility 4 (`xterra-treadmill-warranty-consumer-responsibilities-five-items-with-a-registration-card`).
