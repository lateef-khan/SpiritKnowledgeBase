---
id: xt-2023-warranty-owner-costs
title: What the owner pays even when the repair is under warranty
kind: policy
question: Does the owner of a Spirit XT treadmill pay anything for a warranty repair?
asked_as:
- do i have to pay for a warranty service call
- is the service call free under warranty
- who pays shipping for warranty parts
- why am i being charged a trip fee
keywords:
- trip charge
- service call
- freight charges
- shipping and handling
- 100 miles
- out of pocket
- service call reimbursement
- in-home service
facets:
  brand:
  - spirit
  product_line: treadmill
  model: '*'
  applies_to:
  - xt185-2015
  - xt185-2023
  - xt285-2015
  - xt285-2023
  - xt385-2015
  - xt385-2023
  - xt485-2015
  - xt485-2023
  - xt485ent-2023
  - xt685-2015
  - xt685-2023
  - xt685ent-2023
  section: warranty
  code: '*'
authority: 3
not_to_be_confused_with: []
see_also:
- xt-2023-warranty-service-steps
- xt-2023-warranty-exclusions
- xt-2023-warranty-consumer-responsibilities
source:
  ref: spirit-treadmill-xt185-2023-owners-manual
  locator: 'SERVICE step 4, p. 42; RESPONSIBILITIES OF THE CONSUMER item 3, p. 40;
    EXCLUSIONS item 2, p. 41. All three are in all five 2023 XT manuals: xt185 pp.
    40-42, xt285 pp. 48-50, xt385 pp. 53-55, xt485 pp. 53-55, xt685 pp. 52-54. All
    three are word for word the same in the five 2015 XT manuals, where the accessibility
    item is numbered 4 rather than 3: xt185 text.md lines 765-766, 775-778 and 799-803.
    XT485ENT pp. 55-56 (accessibility item 4 at line 1100, exclusion 2 at line 1109,
    SERVICE step 4 at lines 1133-1137). XT685ENT pp. 53-55'
  extracted_at: '2026-09-09'
---

Yes. The XT manuals - the 2015 generation, the 2023 one, and both ENT manuals alike - name five charges the customer carries while the machine is still inside its warranty period, and refuse one reimbursement outright.

| What the customer pays | Where the manual says so |
|---|---|
| Freight **to and from** a service center, if the product is shipped to one | SERVICE step 4 |
| **Shipping and handling** on replacement parts shipped while the product is under warranty | SERVICE step 4 |
| A **trip charge** for in-home service | SERVICE step 4 |
| An **additional trip charge** if the customer is over **100 miles** from the nearest service center | SERVICE step 4 |
| Expenses for making the treadmill accessible for servicing | Responsibilities of the consumer, item 3 (item **4** in the 2015 manuals and in the XT485ENT, which carry an extra registration item at the top of the list) |

And exclusion 2 refuses **service call reimbursement to the consumer**.

**The manual prints no figure for any of these charges** and no way to get them waived. It also does not say who sets the trip charge.

Adequate packaging for a return is the owner's responsibility too, and freight damage claims go to the carrier, not to Spirit Fitness.

These sentences are the sharpest difference between the XT warranty and the Spirit commercial one: the CT900 and CTSBS900 service steps stop at shipping and handling and say nothing about trip charges. `xt-2023-warranty-service-steps` holds the full step order.
