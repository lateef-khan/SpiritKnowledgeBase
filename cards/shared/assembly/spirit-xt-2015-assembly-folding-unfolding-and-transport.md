---
id: spirit-xt-2015-assembly-folding-unfolding-and-transport
title: Folding, unfolding and rolling away on the locking telescoping tube and the
  unlocking lever
kind: procedure
question: How do I fold, unfold and move a Spirit XT185, XT285, XT385, XT485 2013
  or 2015, or XT485ENT treadmill?
asked_as:
- how do i fold my xt treadmill up
- how do i get the deck back down on my spirit xt
- can i roll the treadmill away folded
- where is the unlocking lever
keywords:
- folding
- unfolding
- locking telescoping tube
- unlocking lever
- transport wheels
- moving the treadmill
- minimum incline
- storage
facets:
  brand:
  - spirit
  product_line: treadmill
  model: '*'
  applies_to:
  - xt185-2015
  - xt285-2015
  - xt385-2015
  - xt485-2013
  - xt485-2015
  - xt485ent-2023
  section: assembly
  code: '*'
authority: 3
not_to_be_confused_with:
- xt-2023-assembly-folding-and-moving
- xt685-2023-moving-the-treadmill
- spirit-ct900-assembly-transportation-instructions
see_also:
- xt-2015-assembly-no-setting-up-section
- xt485ent-2023-assembly-no-setting-up-section
- xt-2023-assembly-folding-and-moving
- f65-2016-folding-and-unfolding
- tt8-2016-ac-folding-and-unfolding
source:
  ref: spirit-treadmill-xt285-2015-service-manual
  locator: '9. Treadmill Folding/Unfolding and Transport: XT285 2015 PDF p. 65 (printed
    59), text.md lines 1298-1326; XT185 2015 PDF p. 64 (printed 59), lines 1227-1250;
    XT385 2015 PDF p. 64, lines 1046-1065; XT485 2015 PDF p. 65, lines 1054-1073;
    XT485ENT PDF p. 59, lines 869-888. The XT385 2015, XT485 2015 and XT485ENT pages
    were also rendered and OCRed to confirm no transport paragraph is printed | xt485-2013
    (spirit-treadmill-xt485-2013-owners-manual): FOLDING INSTRUCTIONS and TRANSPORT,
    PDF p. 13 (printed 12); text.md lines 362-395'
  extracted_at: '2026-09-11'
---

**The owner's manuals for these machines say nothing about folding. The service manuals do.** This is the
service-manual page, printed in five books.

**Before moving it** (four of the five books): do not attempt to move the unit unless it is in the folded and
locked position. Be sure the power cord is secured to avoid possible damage. Use both handrails to manoeuvre
the unit to the desired position.

**To fold**: make certain the treadmill is at **minimum incline**. Lift the running deck until it is secured by
the **locking telescoping tube assembly** in the centre back of the base.

**To unfold**: apply slight forward pressure on the running deck with one hand - at the rear roller area, to
relieve pressure on the locking system. Pull down on the **unlocking lever** and slowly lower the running deck
to the floor. The deck will lower unassisted when it reaches about waist high.

**Transport** (XT185 and XT285 2015 books only): the treadmill is equipped with **four transport wheels**
that are engaged when the treadmill is folded. After folding, simply roll the treadmill away.

What each book prints:

| Book | "Do not move unless folded" preamble | Fold and unfold | Transport paragraph |
|---|---|---|---|
| XT185 2015 | no | yes, with figure numbers 1-4 | yes |
| XT285 2015 | yes | yes | yes |
| XT385 2015 | yes | yes | **no** |
| XT485 2015 | yes | yes | **no** |
| XT485ENT | yes | yes | **no** |

The XT385, XT485 and XT485ENT pages end after the unfold step; there is no wheel paragraph on them, checked
on the rendered page. The XT185 book heads its first section "Unfoldong The Treadmill".

The 2023 XT machines release the deck a different way - a foot on the tube at a yellow sticker and a release
lever - and are on their own card. The Sole F-series books print this same page for the same Dyaco frame.

## The XT485 2013 owner's manual prints the whole page

**The XT485 2013 owner's manual is the one owner's manual on this card that prints it** - under FOLDING INSTRUCTIONS and TRANSPORT, between the assembly steps and the features page, with all three parts: the "do not attempt to move the unit unless it is in the folded and locked position" preamble, the fold and unfold steps word for word (minimum incline, the locking telescoping tube assembly, slight forward pressure at the rear roller area, the unlocking lever, the deck lowering unassisted from about waist high), and the transport paragraph: **four transport wheels that are engaged when the treadmill is folded; after folding simply roll the treadmill away**. Its table of contents lists it as "Transport Instructions 12".
