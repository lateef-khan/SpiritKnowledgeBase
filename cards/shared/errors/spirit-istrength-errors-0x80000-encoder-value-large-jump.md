---
id: spirit-istrength-errors-0x80000-encoder-value-large-jump
title: '0x80000: Encoder Value Large Jump on the digital resistance module'
kind: troubleshooting
question: What does fault code 0x80000 mean on the Spirit i-Strength digital resistance
  module?
asked_as:
- what does 0x80000 mean on my spirit strength machine
- i-strength module fault 0x80000
- encoder value large jump on the resistance module
keywords:
- '0x80000'
- encoder value large jump
- fault code
- hex code
...
**This is `0x80000`, and it is not `0x80`, `0x800`, `0x800000` - the same digit with a different number of zeroes, each a different fault.** Count the zeroes before you act.
...
The encoder is the magnetic encoder inside the servo motor (the specification chapter names it as the motor's position feedback); its cable is one of the four the disassembly chapter has you unplug from the motor - communication, power, ground and temperature sensing - so "check encoder connections" means that connector and the motor's own plug.

Sole's SRVO resistance unit prints the same twelve codes, worded differently, in its own service manual: `srvo-error-0x80000-encoder-value-error` is this code's Sole twin. That card describes a different brand's product and is linked for comparison only.
