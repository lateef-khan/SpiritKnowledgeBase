---
id: e25-2023-chest-belt-no-pulse
title: Chest belt shows no pulse
kind: troubleshooting
question: Why is the wireless chest belt not reading on a Sole elliptical?
asked_as:
- chest strap not reading on sole elliptical
- wireless heart rate not working
- what battery does the sole chest belt take
keywords:
- chest belt
- chest strap
- wireless
- no pulse
- cr2032
- battery
- receiver
- 3 feet
facets:
  brand:
  - sole
  product_line: elliptical
  model: '*'
  applies_to:
  - e25-2023
  - e35-2023
  - e95-2023
  - e95s-2023
  - e98-2023
  section: errors
  code: '*'
authority: 3
not_to_be_confused_with: []
see_also:
- e25-2023-hand-pulse-not-working
- e25-2023-erratic-pulse-display
- e35-2023-hand-pulse-not-working
source:
  ref: sole-elliptical-e25-2023-service-manual
  locator: Troubleshooting procedure matrix, page 29
  extracted_at: '2026-09-04'
---

Symptom row: **Wireless lost its function (no pulse displayed on monitor).**

| Reason | Solve |
|---|---|
| 1. Chest belt not worn properly. | 1. Check the chest belt has proper contact with skin and is oriented correctly. |
| 2. Distance is too far and exceeds range of receiver. | 2. Use the chest belt in front of the console within **3 feet**. |
| 3. Chest belt battery is weak or dead. | 3. Replace with a new lithium battery, type **CR2032**. |

The matrix carries a fourth row, "Chest belt too close to the Elliptical", whose reason is a weak battery and whose fix is again a new **CR2032** lithium battery. The condition text does not match the reason; it is printed that way.

The hand grip sensors have their own row and their own card.
