---
id: spirit-istrength-errors-0x4000000-uvw-wiring-error
title: '0x4000000: UVW Wiring Error on the digital resistance module'
kind: troubleshooting
question: What does fault code 0x4000000 mean on the Spirit i-Strength digital resistance
  module?
asked_as:
- what does 0x4000000 mean on my spirit strength machine
- i-strength module fault 0x4000000
- uvw wiring error on the resistance module
keywords:
- '0x4000000'
- uvw wiring error
- fault code
...
**This is `0x4000000`, and it is not `0x40`, `0x400`, `0x40000`, `0x400000`, `0x40000000` - the same digit with a different number of zeroes, each a different fault.** Count the zeroes before you act.
...
UVW are the three motor phases. The disassembly chapter warns that once the motor's red, black and blue terminals are unplugged they must be kept apart - if any two touch, the winding wheel will not move - which is the same three wires this code is about.

Sole's SRVO resistance unit prints the same twelve codes, worded differently, in its own service manual: `srvo-error-0x4000000-uvw-cord-error` is this code's Sole twin. That card describes a different brand's product and is linked for comparison only.
