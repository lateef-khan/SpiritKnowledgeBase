---
id: sole-treadmill-errors-e3-owner-checklist
title: 'E3 incline error: owner''s checklist'
kind: troubleshooting
question: What should I check when a Sole treadmill shows E3?
asked_as:
- e3 incline error
- treadmill incline not working e3
- what is e3 on sole
keywords:
- e3
- incline error
- incline motor
- cable
- vr
- stuck mechanism
- intermittence
facets:
  brand:
  - sole
  product_line: treadmill
  model: '*'
  section: errors
  applies_to:
  - f80-2023
  - f85-2020
  - f85-2021
  - f85-2023
  - f89-2023
  - tt8-2021
  - tt8-2023
  code: e3
authority: 3
not_to_be_confused_with:
- sole-treadmill-errors-e1-owner-checklist
- sole-treadmill-errors-e2-owner-checklist
- sole-treadmill-errors-e4-owner-checklist
- sole-treadmill-errors-e5-owner-checklist
- sole-treadmill-errors-e6-owner-checklist
- sole-treadmill-errors-e7-owner-checklist
- f63-2026-e3-error-code
see_also:
- f65-2023-e3-incline-vr-voltage
- sole-dc-controller-error-code-list
source:
  ref: sole-tm-f80-2023-owners-manual
  locator: 'ERROR MESSAGES FOR DIGITAL-CONTROL SYSTEM TREADMILLS: sole-tm-f80-2023-owners-manual
    PDF p. 31 (printed 29); sole-tm-f85-2021-owners-manual PDF p. 33 (printed 31);
    sole-tm-f85-2023-owners-manual PDF p. 31; sole-tm-f89-2023-owners-manual PDF p.
    32; sole-tm-tt8-2021-owners-manual PDF p. 33 (printed 31); sole-tm-tt8-2023-owners-manual
    PDF p. 29 (printed 27)'
  extracted_at: '2026-09-12'
---

**This is the stencil-era owner's-manual E3 (incline error), not the 2026 E3 and not Spirit's E3.**

The six owner's manuals print the same ERROR MESSAGES table with OK/NG checkboxes. The F80-2023, F85-2023 and F89-2023 prints read 'E1_'..'E7_' on the OCR layer — the render shows a plain code beside the checkbox column.

| ERROR | MEANING | POSSIBLE CAUSE |
|---|---|---|
| Console showing E3 | Incline Error | 1. Is the cable between incline motor and controller connected correctly and properly? 2. Check if incline mechanism being stock [sic] or defective? 3. Is VR connected properly or with inter-mittance? |

The book prints 'stock' where it means stuck (render and OCR agree). The service manuals' deeper E3 procedure (VR voltage range) is a separate card: see `f65-2023-e3-incline-vr-voltage`.
