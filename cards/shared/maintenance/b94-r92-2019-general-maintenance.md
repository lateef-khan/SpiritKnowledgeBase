---
id: sole-b94-r92-2019-general-maintenance
title: Squeaks, thumps and levelling
kind: troubleshooting
question: My Sole bike or squeaks or thumps. What do I check?
asked_as:
- my sole bike is making a clicking noise
- exercise bike squeaks when i pedal
- bike feels rough when pedaling
keywords:
- squeak
- thump
- clicking
- rough
- loose hardware
- crank arm nut
- levelling
- 14mm
facets:
  brand:
  - sole
  product_line: bike
  model: '*'
  applies_to:
  - b94-2016
  - b94-2019
  - b94-2023
  - r92-2016
  - r92-2019
  - r92-2023
  section: maintenance
  code: '*'
authority: 3
not_to_be_confused_with: []
see_also:
- b94-2019-owners-manual-assembly-step-4
- r92-2019-owners-manual-assembly-step-4
source:
  ref: sole-bike-b94-r92-2019-owners-manual
  locator: page 31, General Maintenance
  extracted_at: '2026-09-08'
---

Wipe down all areas in the sweat path with a damp cloth after each workout.

If a squeak, thump, clicking or rough feeling develops, the main cause is most likely one of:

1. **Hardware not sufficiently tightened during assembly.** All bolts installed during assembly need to be tightened as much as possible. It may be necessary to use a larger wrench than the one provided. **Ninety percent of calls to the service department for noise issues can be traced to loose hardware.**
2. **The crank arm nut needs to be retightened.**
3. If squeaks or other noises persist, check the unit is properly levelled. There are **2 levelling pads on the bottom of the rear stabilizer**. Use a **14mm wrench** (or adjustable wrench) to adjust the levellers.

A clicking or thumping specifically while pedalling is usually loose pedals — the assembly instructions say to tighten pedals as much as you possibly can, and to re-tighten them if thumping appears.
