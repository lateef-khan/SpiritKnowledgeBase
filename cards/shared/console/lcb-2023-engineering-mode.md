---
id: lcb-2023-engineering-mode
title: Getting into engineering mode
kind: procedure
question: How do I get into engineering mode on a Sole bike?
asked_as:
- how do i get into engineering mode on my 2023 lcb
- service menu on the light commercial upright bike
- engineer mode on a 2023 sole touchscreen bike
keywords:
- engineering mode
- engineer mode
- settings page
- service menu
- diagnostic
- touchscreen
- ten taps
facets:
  brand:
  - sole
  product_line: bike
  model: '*'
  applies_to:
  - lcb-2023
  - lcr-2023
  section: console
  code: '*'
authority: 3
not_to_be_confused_with: []
see_also:
- lcb-2023-console-modes
- sole-engineering-menu-touchscreen
- sole-bike-engineering-mode-tft-consoles
- lcr-2023-console-modes
source:
  ref: sole-bike-lcb-2023-service-manual
  locator: Section 8.6 Maintenance Menu, page 21
  extracted_at: '2026-09-04'
---

Go to the **Settings** page and **press 10 times on the word "Settings"** to enter engineering mode.

That is the whole of section 8.6. **The manual does not list what is inside the menu**, only how to open it. The LCB 2016, LCB 2019 and LCR 2016 manuals list a seven item menu reached by holding Start, Stop and Enter; that key combination does not apply to this console.
