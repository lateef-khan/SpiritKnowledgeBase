---
id: sole-treadmill-safety-operating-environment-limits
title: Rain, moisture, pools and the temperature and humidity limits
kind: policy
question: Where may a Sole treadmill be used, and what are its environment limits?
asked_as:
- treadmill in garage ok
- treadmill humidity limit
- treadmill max temperature
- can treadmill go outside
keywords:
- rain
- moisture
- outdoors
- pool
- spa
- humidity
- temperature
- 40 degrees
- 95 percent
- non-condensing
facets:
  brand:
  - sole
  product_line: treadmill
  model: '*'
  applies_to:
  - af63-2011
  - af65-2011
  - af80-2011
  - af83-2011
  - af85-2011
  - as77-2012
  - att8-2011
  - f60-2013
  - f60-2014
  - f60-2016
  - f60-2020
  - f63-2006
  - f63-2013
  - f63-2015
  - f63-2016
  - f63-2019
  - f63-2023
  - f63-2025
  - f65-2013
  - f65-2014
  - f65-2016
  - f65-2019
  - f65-2023
  - f80-2006
  - f80-2013
  - f80-2016
  - f80-2019
  - f80-2023
  - f83-2006
  - f85-2006
  - f85-2013
  - f85-2016
  - f85-2019
  - f85-2020
  - f85-2021
  - f85-2023
  - f89-2023
  - s73-2006
  - s77-2006
  - s77-2013
  - s77-2016
  - s77-2019
  - st90-2020
  - st90-2021
  - st90-2023
  - td80-2019
  - tt8-2006
  - tt8-2013
  - tt8-2016
  - tt8-2016-ac
  - tt8-2019
  - tt8-2020
  - tt8-2021
  - tt8-2023
  - tt8c-2011
  - tt9-2018
  - uf63-2006
  - uf80-2006
  - uf83-2006
  - uf85-2006
  - us73-2006
  - us77-2006
  - utt8-2006
  - vf63-2007
  - vf80-2007
  - vf83-2007
  - vf85-2007
  - vs73-2007
  - vs77-2007
  - vtt8-2007
  - wf63-2009
  - wf63-2010
  - wf80-2009
  - wf80-2010
  - wf83-2009
  - wf83-2010
  - wf85-2009
  - wf85-2010
  - ws73-2009
  - ws73-2010
  - ws77-2009
  - ws77-2010
  - wtt8-2009
  - wtt8-2010
  section: safety
  code: '*'
authority: 3
not_to_be_confused_with: []
see_also:
- f50-2026-operating-environment-limits
- f63-2026-operating-environment-limits
source:
  ref: sole-tm-f63-2013-2015-owners-manual
  locator: Electrical page, PDF p. 5 (p. 6 in the AF63/AF80/AS77/ATT8/TT8C books) - A2 same paragraph, electrical page PDF p. 5, in all 24 books.
  extracted_at: '2026-09-12'
---

**NEVER expose this treadmill to rain or moisture. This product is NOT designed for use outdoors, near a pool or spa, or in any other high humidity environment.** The temperature/humidity figure varies:

| Books | Figure |
|---|---|
| All 24 A2 stencil books (2016-2025) | The **maximum operating** temperature specification is 40 degrees C, humidity 95% non-condensing (no water drops forming on surfaces). The TD80-2019 book drops the leading `The`. |
| All 24 A2 stencil books (2016-2025) | The **maximum operating** temperature specification is 40 degrees C, humidity 95% non-condensing (no water drops forming on surfaces). The TD80-2019 book drops the leading `The`. |
| Most 2009-and-later books (AF, AS77, ATT8, F60, F63-2013, F65, F80-2013, S77, TT8-2013, TT8C, TT9, WF63, WF80, WTT8, UF80) | The temperature specification is **40 degrees C, and humidity is 95% non-condensing** (some add "no water drops forming on surfaces"). |
| VF63, VF80/VF83/VF85 2007 printings | The **maximum** temperature specification is 40 degrees C, humidity 95% non-condensing. |
| VS73/VS77, VTT8, WS73/WS77 | The temperature specification is **40 to 120 degrees Fahrenheit, and humidity limit is 95%**, non-condensing. |
| 2006 families (F63/UF63, F80 family, TT8/UTT8) | No temperature or humidity figure at all; only the high-humidity-environments sentence. |

Whether the warranty stays valid outside a climate-controlled room belongs to the warranty section.
