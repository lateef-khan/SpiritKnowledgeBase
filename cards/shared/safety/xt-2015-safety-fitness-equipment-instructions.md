---
id: xt-2015-safety-fitness-equipment-instructions
title: The Fitness Equipment Safety Instructions list
kind: policy
question: What does the Fitness Equipment Safety Instructions list say for a Spirit XT185-2015, XT285-2015, XT385-2015, XT485-2015 or XT685-2015 treadmill, or for a Spirit XT485ENT treadmill?
asked_as:
- what shoes should i wear on the treadmill
- do i need to see a doctor before using a treadmill
- can i put the treadmill on carpet
- are the hand grips accurate
keywords:
- fitness equipment safety
- proper shoes
- athletic shoes
- consult a physician
- age 35
- shag carpet
- moving parts
- pulse sensors
- not a medical device
- longevity
facets:
  brand:
  - spirit
  product_line: treadmill
  model: '*'
  applies_to:
  - xt185-2015
  - xt285-2015
  - xt385-2015
  - xt485-2015
  - xt485ent-2023
  - xt685-2015
  section: safety
  code: '*'
authority: 3
not_to_be_confused_with:
- xt-2023-safety-instructions-list
see_also:
- xt-2015-safety-instructions-list
- xt-2015-safety-placement-requirements
- xt-2015-safety-who-may-use-the-treadmill
source:
  ref: spirit-treadmill-xt185-2015-owners-manual
  locator: page 5, Fitness Equipment Safety Instructions; the XT285 and XT485 manuals
    print the same list on page 5, the XT385 and XT685 manuals on page 6. The XT485ENT owner's manual prints the same list under the same heading, page 5
  extracted_at: '2026-09-09'
---

A second, separately headed list, printed on the page after the Important Safety Instructions
(`xt-2015-safety-instructions-list`).

- **Do not operate equipment on deeply padded, plush or shag carpet.** Damage to both carpet and
  equipment may result.
- **Before beginning this or any exercise program, consult a physician.** This is especially
  important for persons over the age of 35 or persons with pre-existing health conditions.
- **Do not attempt to use your equipment for any purpose other than for the purpose it is
  intended.**
- **Keep hands away from all moving parts.**
- **The pulse sensors are not medical devices.** Their purpose is to provide you with an
  approximate measurement in relation to your target heart rate. Various factors, including the
  user's movement, may affect the accuracy of heart rate readings. The pulse sensors are intended
  only as exercise aids in determining heart rate trends in general.
- **Wear proper shoes.** High heels, dress shoes, sandals or bare feet are not suitable for use on
  your equipment. Quality athletic shoes are recommended to avoid leg fatigue.

The list closes with its own consequence line:

> Failure to follow all guidelines may compromise the effectiveness of the exercise experience,
> expose yourself (and possibly others) to injury, and reduce the longevity of the equipment.

**The pulse-sensor bullet is not the same in all five manuals.** The XT285-2015, XT385-2015,
XT485-2015 and XT685-2015 manuals add one sentence the **XT185-2015** manual does not print:

> Use of a chest transmitter strap (sold separately) is a much more accurate method of heart rate
> analysis.

The XT185-2015 manual makes no comparison between the grips and a chest strap, so do not quote
that sentence for an XT185.

**The carpet rule is printed only here.** These 2015 manuals have no separate location or
clearance page; what they do and do not say about where the machine may stand is gathered in
`xt-2015-safety-placement-requirements`.

**By 2023 this list was merged away.** The 2023 XT manuals fold these bullets into a single
numbered safety list as items 15-20 and drop the chest-strap sentence from the pulse-sensor item
(`xt-2023-safety-instructions-list`). The rules themselves are unchanged.

**The XT485ENT owner's manual prints this separately headed list word for word**, on its page 5,
including the closing consequence line. Its pulse-sensor bullet is the longer one: it carries
`Use of a chest transmitter strap (sold separately) is a much more accurate method of heart rate
analysis`, as the XT285, XT385, XT485 and XT685 2015 manuals do and the XT185-2015 manual does not.

**The XT685ENT manual has no such list.** Like the base 2023 manuals it folds these bullets into
its single numbered safety list as items 15 to 20 and drops the chest-strap sentence
(`xt-2023-safety-instructions-list`).
