---
id: lcb-2023-model-overview
title: 'What this light commercial upright bike is: factory code and
  drive system'
kind: spec
question: What is the Sole LCB-2023, and what console and resistance does
  it use?
asked_as:
- what is the sku for the lcb 2023
- what console does the 2023 lcb use
- how does resistance work on the 2023 light commercial upright bike
keywords:
- model number
- sku
- factory code
- service manual
- console
- resistance
- ems brake
- upright bike
facets:
  brand:
  - sole
  product_line: bike
  model: lcb-2023
  applies_to:
  - lcb-2023
  section: specs
  code: '*'
  model_number:
  - '511122'
authority: 3
not_to_be_confused_with: []
see_also:
- sole-bike-service-manual-model-numbers
- sole-bike-ems-brake-spec
- lcb-model-numbers
source:
  ref: sole-bike-lcb-2023-service-manual
  locator: Cover page, Section 3 Electrical Configurations and Section 4 Product Operation
  extracted_at: '2026-09-04'
---

| Field | Value |
|---|---|
| Code on the service manual cover | SU615A-SB026 |
| Product name on the cover | ENT UPRIGHT BIKE |
| Console | Touchscreen with a keypad and a TFT LCD. The brake controller carries the DC power supply for the console and the EMS driver control circuit. |
| Resistance | EMS brake, DC 0 to 21 V, 40 resistance levels. |
| Family | Light commercial upright bike, drive belt with a J-bolt tensioned idle arm and an induction brake / generator flywheel. |

The manual covers outlines, electronic parts, wiring, safety, three error messages, ten replacement procedures and three troubleshooting answers.

The console to brake controller cable is a **6-pin computer cable**, and the brake controller part number the manual asks you to verify is **CS51012**.

The manual prints only the factory code. The model number is on the LCB model card, `lcb-model-numbers`.
