---
id: ct850-2016-tread-belt-lubrication
title: Whether to lubricate the tread belt
kind: fact
question: Should I lubricate the tread belt on a Spirit ct850-2016 treadmill?
asked_as:
- do i need to lubricate this treadmill belt
- what lubricant goes on the spirit ct850 deck
- how often should i lube the running belt
keywords:
- lubrication
- silicone lubricant
- waxed deck
- no cleaners or abrasives
- missing section
- dangling cross reference
facets:
  brand:
  - spirit
  product_line: treadmill
  model: ct850-2016
  applies_to:
  - ct850-2016
  section: maintenance
  code: '*'
  model_number:
  - '850845'
authority: 3
not_to_be_confused_with:
- sole-lubricate-running-belt
see_also:
- ct850-2016-tread-belt-tension-adjustment
- ct850-2016-tread-belt-and-deck-cleaning
- ct850-tread-belt-hesitates-when-stepped-on
- ct900ent-belt-deck-cleaning
- ct900-treadbelt-off-center-or-hesitates
- spirit-ct800-maintenance-belt-deck-cleaning-4000-hours
source:
  ref: spirit-treadmill-ct850-2016-service-manual
  locator: Section 8.3 troubleshooting matrix, p. 52 (printed 51) against section
    9.1 Tread belt and Deck, p. 54 (printed 53); the same matrix row is in CT850 2020
    section 8-7, p. 51 (printed 50)
  extracted_at: '2026-09-08'
---

**This manual contradicts itself, and the procedure it points at does not exist.** Read the whole
card before applying anything to the belt or deck.

## What the troubleshooting matrix says

> Tread belt hesitates while being stepped on. — **Insufficient lubricant on tread belt.** Tread
> belt tension insufficient. — **See treadmill belt lubrication**

## What the maintenance section says

Section 9.1 Tread belt and Deck gives the only belt and deck care this manual prints. It says to
wipe with a damp cloth and **"Use water only - no cleaners or abrasives"**. It never mentions
lubricant, wax or a lubrication interval.

## The section being referred to does not exist

There is **no "treadmill belt lubrication" section** anywhere in this manual. Section 9 has only
9.1 Tread belt and Deck and 9.2 Service Troubleshooting Checklist. The identical matrix row in the
CT850 2020 manual is worse: that manual has no General Maintenance chapter at all.

## The owner's manual settles it: no lubricant

The CT850 2016 owner's manual (p. 39, BELT/DECK) describes the same machine and says plainly:
**"Do not apply any type of lubricant or wax to the surface."** The CT850 2020, CT850 2018 and
CT850ENT 2022 owner's manuals print the same sentence. That is the answer to give -
`spirit-ct800-maintenance-belt-deck-cleaning-4000-hours` for the 2016, 2020 and ENT 2022 decks,
`ct900-belt-deck-cleaning` for the 2018 hard wax deck. The service manual's matrix row is the
outlier, and the section it points at still does not exist.

## What to do

Treat the hesitation as a **tension** problem first —
`ct850-2016-tread-belt-tension-adjustment` — and clean per
`ct850-2016-tread-belt-and-deck-cleaning`. Do not apply lubricant.

**Do not apply a Sole treadmill lubrication procedure to this machine.** Elsewhere in the Spirit
commercial range the deck is a hard waxed deck whose manual says "Do not apply any type of
lubricant or wax to the surface" (`ct900ent-belt-deck-cleaning`), and the CT900 manual records the
same self-contradiction as this one (`ct900-treadbelt-off-center-or-hesitates`).
