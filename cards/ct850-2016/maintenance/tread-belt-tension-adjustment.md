---
id: ct850-2016-tread-belt-tension-adjustment
title: Adjusting tread belt tension
kind: procedure
question: How do I adjust the tread belt tension on a Spirit ct850-2016 treadmill?
asked_as:
- how do i tighten the running belt
- the belt slips when i walk on it
- how do i adjust tread belt tension
keywords:
- belt tension
- rear roller bolts
- step rail end caps
- quarter turn
- belt slipping
- over tightening
- motor drive belt
facets:
  brand:
  - spirit
  product_line: treadmill
  model: ct850-2016
  applies_to:
  - ct850-2016
  section: maintenance
  code: '*'
authority: 3
not_to_be_confused_with: []
see_also:
- ct850-2016-treadbelt-tracking-adjustment
- ct850-2016-tread-belt-and-deck-cleaning
- ct850-tread-belt-hesitates-when-stepped-on
- ct850-noise-under-motor-cover
- ct900ent-belt-tension-adjustment
source:
  ref: spirit-treadmill-ct850-2016-service-manual
  locator: Section 9.1 BELT ADJUSTMENTS, pp. 54-55 (printed 53-54)
  extracted_at: '2026-09-08'
---

Adjustment must be made **from the rear roller**. The adjustment bolts are at the end of the step
rails, in the end caps. The manual notes: "Adjustment is through small hole in the end cap."

1. Tighten the rear roller bolts **only enough to prevent slippage at the front roller**.
2. Turn **both** tread-belt tension adjustment bolts in increments of **1/4 turn each**.
3. Inspect for proper tension by walking on the belt at a low speed, making sure the belt does not
   slip.
4. Keep tensioning the bolts until the belt stops slipping.

**DO NOT OVERTIGHTEN — over tightening will cause belt damage and premature bearing failure.**

## If it is tight and still slips

The manual says directly: "If you feel the belt is tight enough, but it still slips, the problem
may be a loose Motor drive belt under the front cover." That is the same cause the troubleshooting
matrix gives for a noise under the motor cover — see `ct850-noise-under-motor-cover`.

The same bolts set tracking, and turning them unevenly walks the belt off centre — see
`ct850-2016-treadbelt-tracking-adjustment`.

## The owner's manual prints the same procedure with a wrench size

The CT850 2016 owner's manual (p. 39) gives the same rear-roller, 1/4-turn-each procedure and adds
what this manual leaves out: an **8 mm Allen wrench**, the qualifier "belt tension is not critical
for most users", and the caution to turn the bolts on both sides evenly. It is carded at
`ct900ent-belt-tension-adjustment`, which covers all four CT850 owner's-manual years. Note that the
same owner's manual's tools list names a **10 mm** Allen wrench and no 8 mm one, so treat the 8 mm
figure with care.

**The CT850 2020 *service* manual has no General Maintenance chapter.** Its troubleshooting row for
a hesitating belt points at a "General Maintenance section on running belt Tension" that is not in
that document. Among the service manuals this procedure is printed only in the 2016 one; all four
CT850 owner's manuals print their own version.
