---
id: ct900ent-facility-responsibilities
title: Normal responsibilities of the facility (warranty)
kind: policy
question: What is a facility responsible for under the Spirit CT900ENT, 4.0T, 7.0T
  or 8.0T warranty?
asked_as:
- what am i responsible for under the warranty
- what do i need to do to keep my warranty valid
keywords:
- facility responsibilities
- warranty registration card
- proper installation
- grounded power supply
- routine maintenance
facets:
  brand:
  - spirit
  product_line: treadmill
  model: '*'
  applies_to:
  - 40t-2026
  - 70t-2026
  - 80t-2026
  - ct900ent
  section: warranty
  code: '*'
authority: 3
not_to_be_confused_with:
- xt-2023-warranty-consumer-responsibilities
- xt-2015-warranty-consumer-responsibilities
see_also:
- ct900ent-warranty-coverage-terms
- ct900ent-serial-number-and-product-registration
- spirit-2026t-warranty-commercial-periods
- spirit-2026t-warranty-registration
- spirit-2026t-warranty-owner-costs
source:
  ref: ct900ent-om
  locator: 'p. 50. The same seven items, word for word and in the same order, are printed
    under RESPONSIBILITIES OF THE FACILITY in the three 2026 manuals: 4.0T p. 51, 7.0T
    p. 50, 8.0T p. 68'
  extracted_at: '2026-08-24'
---

The facility is responsible for the items listed below:

1. The warranty registration card must be completed and returned to the address listed on the card within 10 days of the original purchase, or completed online, to validate the manufacturer's limited warranty.
2. Proper use of the fitness equipment in accordance with the instructions provided in this manual.
3. Proper installation in accordance with instructions provided with the fitness equipment and with all local electric codes.
4. Proper connection to a grounded power supply of sufficient voltage, replacement of blown fuses, repair of loose connections or defects in house or facility wiring.
5. Expenses for making the fitness equipment accessible for servicing, including any item that was not part of the fitness equipment at the time it was shipped from the factory.
6. Damages to the fitness equipment finish during shipping, installation or following installation.
7. Routine maintenance of this unit as specified in this manual.

**The 2026 4.0T, 7.0T and 8.0T print this list unchanged**, under the same heading, on the same page as their Commercial warranty table. One difference is typographic only: those manuals number the lead-in sentence "The facility is responsible for the items listed below:" as item 1, so their list runs to eight numbers for seven duties.

**Two other Spirit commercial manuals print a shorter registration duty.** The CT900 and the CTSBS900 open the same list with "The warranty registration **must be completed online**" and impose no day count. The ten-day card deadline in item 1 above belongs to the CT900ENT and the three 2026 machines only - see `spirit-2026t-warranty-registration`.

**Do not confuse this with the XT list.** The residential XT manuals head their version RESPONSIBILITIES OF THE CONSUMER, run to five items, open with "the warranty applies only to products in ordinary household use", and carry no grounded-power-supply item at all: `xt-2023-warranty-consumer-responsibilities`.
