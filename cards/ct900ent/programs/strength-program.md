---
id: ct900ent-strength-program
title: Strength program
kind: fact
question: What is the Strength program on a Spirit treadmill and how do I use it?
asked_as:
- what does the strength program do
- which program builds leg strength
keywords:
- strength program
- muscular endurance
- gradual progression
- lower body
facets:
  brand:
  - spirit
  product_line: treadmill
  model: '*'
  applies_to:
  - ct900ent
  - xt185-2023
  - xt285-2023
  - xt385-2023
  - xt485-2023
  - xt685-2023
  section: programs
  code: '*'
authority: 3
not_to_be_confused_with: []
see_also:
- ct900ent-preset-speed-incline-chart
- xt-2023-programs-select-and-start-xt185-xt285
- xt-2023-programs-select-and-start-xt385-xt485-xt685
source:
  ref: ct900ent-om
  locator: p. 34
  extracted_at: '2026-08-24'
---

**Speed:** This program has a gradual progression of speed up to 100% of maximum effort that is sustained for 25% of workout duration. This will help build strength and muscular endurance in the lower body and glutes. A brief cool down follows.

**Incline:** There is a quick climb to a moderate, sustained elevation that lasts the majority of the workout length.

All five 2023 XT owner's manuals (XT185, XT285, XT385, XT485, XT685) print the same description. The XT185 and XT285 manuals write "gradual progression of resistance" where the CT900ENT and the XT385, XT485 and XT685 write "gradual progression of speed". The CT900ENT manual prints "sustained **from** 25% of workout duration" where all five XT manuals print "sustained **for** 25%"; the CT900ENT wording looks like a typo but is recorded here because the source says it.

On the CT900ENT, the full segment-by-segment speed and incline values are in [Preset program speed/incline chart](preset-speed-incline-chart.md). The 2023 XT manuals print a profile picture only, with no numeric segment table.
