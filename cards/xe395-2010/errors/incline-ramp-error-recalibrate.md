...
asked_as:
- my spirit elliptical says there is an error with the ramp
...
If the console shows an error for the incline/ramp, **re-calibrating the system may resolve the
problem**:

1. Press and hold the **Start** and **resistance level UP** keys for **5 seconds**.
2. The Message Center will show `Calibration`. Press **Enter**.
3. The calibration process is automatic, and the system will **restart** when it has finished.

That key hold is **not** the one that opens the Maintenance Menu. The menu itself is reached with
Start, Stop and Enter held together for about 5 seconds; calibration is item `g` inside it and has
its own two-key combination.

**The manual never prints the wording of the message.** It says only "an error for the
