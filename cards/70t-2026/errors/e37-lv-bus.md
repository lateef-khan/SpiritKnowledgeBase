...
question: What does error code E37 (LV Bus) mean on a Spirit treadmill, and what does
...
source:
  ...
  locator: 'Error Codes table, ERROR MESSAGE & TROUBLESHOOTING, printed page 46; text.md
    column 4 of 5, lines 1498-1500; the same row is printed word for word in both
...
**Service manual remedy.** The 7.0T (MT8000-ST021-02) service manual, section 4.2.3 *Error Codes: Messages, Cause and Solution*, prints this code as **E37 LV Bus: The DC voltage inside Inverter below the minimum allowable value.** and gives, in order:

1) Check the AC line voltage.
2) Replace Inverter.

**The 2024 MT200 error-codes leaflet (`spirit-treadmill-mt200-error-codes-list`) prints it as `E37 Low Voltage Bus` with one line: *Check AC line voltage, check inverter.*** The leaflet is titled for the MT200 with no year and is carried for both MT200 revisions; it is a summary of the list above in fewer words, and where it names parts it names the same ones.
