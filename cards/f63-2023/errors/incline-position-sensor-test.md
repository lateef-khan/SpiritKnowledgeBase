---
id: f63-2023-incline-position-sensor-test
title: Voltage test for the incline position sensor
kind: procedure
question: How do I test the incline position sensor on a Sole F63-2023?
asked_as:
...
source:
  ...
  locator: pages 26 to 28, Test procedure under 8.4
...
3. If it does not, do the Up and Down lights on the controller light?
4. If they light, do the relays click on?
   ...
   - **Light on but the relay does not click**: replace the controller. A bad relay is the likely cause.
...
6. Measure the potentiometer voltage at the 3-pin connector on the controller. No voltage means the wire from the motor to the connector is faulty.
7. If it is there, measure at the output connector to the console at the bottom of the controller. No voltage there means a problem on the controller. There are no electronic components on the board for this signal, only circuit connections, so only a bad solder joint or a broken circuit is possible.
8. If the voltage is present at that output connector, measure at the console. Voltage at the controller but not at the console means the cable between them has a cut or a bad input wire connection.
...
**Console connector wiring**, the same on the controller and at the console:
