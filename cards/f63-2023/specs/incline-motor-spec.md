---
id: f63-2023-incline-motor-spec
title: Incline motor voltage and wiring
kind: spec
question: What are the incline motor specifications for a Sole F63-2023?
asked_as:
- what voltage is the incline motor
- which incline wire is up
- incline motor wiring on my sole treadmill
keywords:
- incline motor
- ac motor
- up wire
- down wire
- com
- position sensor
- neutral
- grounding wire
facets:
  brand:
  - sole
  product_line: treadmill
  model: f63-2023
  applies_to:
  - f63-2023
  section: specs
  code: '*'
  model_number:
  - '563822'
authority: 3
not_to_be_confused_with: []
see_also:
- f63-2023-e3-error-code
- f63-2023-incline-motor-replacement
- f63-2023-incline-position-sensor-test
source:
  ref: sole-tm-f63-2023-service-manual
  locator: page 6, GENERAL INFORMATION, and page 28
  extracted_at: '2026-09-04'
---

A **110 or 230 volt AC motor**, used to control the elevation through the console and the main controller.

**Five wire connection: red, black, white, green**, plus **one 3-pin cable for the position sensor**.

- AC voltage on the **red** wire (UP) increases the incline.
- AC voltage on the **black** wire (DOWN) decreases the incline.
- The **white** wire (COM) is neutral.
- The **green/yellow** wire is the grounding wire.

The 3-pin position sensor connector reads: pin 1 ground, pin 2 position signal 0~5vdc, pin 3 5vdc.
