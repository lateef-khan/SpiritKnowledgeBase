<!-- Source: TR260_OM_20231208.pdf. Text is pdftotext -layout; pages a 300 dpi render knows more about than the text layer are appended below under '=== OCR SUPPLEMENT, PDF PAGE n ===' headers. -->
...
21    MANUFACTURER’S LIMITED WARRANTY
...
Motor Cover & Clean & Inspect Deck Lubricate Deck Inspect Belt Tracking
 Check Wiring    for Lubrication   (Every 40 hours (Monthly) Adjust if      Date
...
has been manufactured by one of the leading fitness manufacturers in the world and is backed
...
purchase for future reference. We appreciate your confidence in XTERRA Fitness and we will
...
TR260 Owner’s Manual
Revision 4.0: 12.08.2023
...
WARNING
To reduce the risk of burns, fire, electric shock, or injury to persons, install the treadmill on a flat level surface with
...
performance of your treadmill may be affected. Such conditions are not covered under your warranty. If you suspect
...
REMOVE TETHER CORD AFTER USE TO PREVENT UNAUTHORIZED TREADMILL OPERATION.
                                                           ...
                                                           6
TR260 ASSEMBLY PACK CHECKLIST




 #79 Ø8 x Ø18 x 1.5T    #87 5/16" x UNC18 x 1/2"     #88 Ø8 x Ø23 x 1.5T
     Flat Washer        Button Head Socket Bolt        Curved Washer
        (6 pcs)                 (12 pcs)                   (6 pcs)
...
■步驟 2.
                                                                         No Hardware
連接控制線中段 (38) 與控制線下段 (39)。                                                 required for
                                                                            this step
...
1.連接控制線中段      (38) 與控制線下段
   Remove the UNIT               (39)。
                    from the box and put it on a flat and level floor.
 Connect the Computer Cable (Middle)(38) with the Computer Cable
 (Lower)(39).
...
Wrench Head Screw Wrench (104). (Please fix the screw without tightening it.)
 ...
 ■    步驟 4                                                                                     Washer (88)
 NOTE: this step may work better with a second person assisting.
  連接控制線上段(37)與控制線中段(38)。                                                                         Qty: 2
1.Connecting
   Insert thetheUPRIGHTS
                Computer Cable(L,
                               (Upper) (37) into
                                  R)(3,4)    with the
                                                  theComputer
                                                      FRAMECable
                                                              BASE (2) with
   the  6 BUTTON HEAD SOCKET BOLTS (87),4 FLAT WASHERS
  (Middle)(38).
   (79), and 2 CURVED WASHERS (88) on the rounded side of the
   upright by using the ALLEN WRENCH. (Please secure the screw
...
  ASSEMBLY STEP 5
 ...
 意 : Be
注NOTE: 請在所有零件組裝完成後，請鎖緊全部的螺絲。
        careful not to pinch the computer cables when
   attaching the console.

NOTE:    Please
 1. Install       Tighten
            the CONSOLE   All Screws
                        SUPPORT (5) onto After  All Components
                                         the UPRIGHTS                  Assembly
                                                      (L,R)(4,3) with the 4     Comple
     BUTTON HEAD SOCKET BOLTS (87) and 4 CURVED WASHERS (88), and 2
     FLAT WASHERS (79) on the flat interior side by using the ALLEN WRENCH.
  ...
  follow the steps on page 18 to lubricate your treadmill belt using approved treadmill belt
  lubricant.
...
                                                          FOLDING INSTRUCTIONS
                                                            ...
                                                            other
              Step: Hold the deck with one hand and pull the knob          hand.
                                                                  outward with the other to let the shaft go out of the deck
              and let the deck go down slowly to the floor.
                                                                                       ...
                                                                                       slowly lower the running deck to the
                                                                                       floor.
   ...
   :一手扶住   跑床，  is一手將
                  equipped
                       拉鈕向外with
                              拉，two transport
                                  使拉鈕軸   心與跑床 wheels.
                                                分離，將    跑 your Treadmill
                                                      Fold
 before
床慢慢  放下至地moving.
            面。 Lean the Treadmill back so that the transport wheels
  touch the floor. You are then free to move the unit.
...
2. Press the Start key to begin belt movement. Adjust to the desired speed using the Speed p
   and q keys. You may also use the rapid SPEED buttons 2, 4, 6, 8, 10 to adjust the speed.
3. To incline the tread-belt press and the the Incline p /q key to the desired incline level. You
   may also press the rapid INCLINE buttons 0, 3, 5, 7, 10 to adjust.
...
1. When the treadmill is running the pause feature may be utilized by pressing the red Stop
   ...
   Calorie readings will hold while the unit is in the pause mode. After five minutes the display
...
MANUAL PROGRAM
In standby mode, you can directly start or press ENTER to set the countdown parameters
(time, distance, calories), Please follow SPEED ▲ and ▼ in order to adjust the time, distance,
calories, confirm and press ENTER key to switch time, distance, calories, the system only
accept the last number of parameters, For example, if the last input is a distance, the previous
parameter will be invalid and only the distance setting will be accepted.

The time preset value is: 30: 00 (5: 00 ~ 99: 00), for each increase, the reduced value is 1:00.

Distance from the default of 5.00 miles (1.00 ~ 99.00), 0.1 increments at a time, the reduced
value is 0.1.

Calories are preset 500 (10 ~ 990), 10 increments each time, the reduced value is 10.

The starting speed is 0.5 miles and the number of ascending segments is zero.
...
the heart beat value is displayed. Grasp firmly while using this feature otherwise the value fluc-
tuates. It will also pick up wireless heart rate transmitters that Bluetooth compatible.


KILOMETER AND MILE SWITCHING FUNCTION
In the standby state, press the speed deceleration key and the program key at the same time,
and the buzzer will sound twice to switch successfully.
...
on page 17 to add lube as needed. After application hold PROGRAM for 3 seconds to reset
the message.
...
The TR260 Treadmill provides 12 programs (P1,P2,P3……P12). Each program has its specific
speed variations.
...
computers via the Xterra + App.

Just download the free Xterra + App from the Apple Store or Google Play, and then follow the
instructions in the App to sync with your exercise machine.

You can press the "DISPLAY" button from the APP to view the display of the current workout
data. When your exercise is complete pressed in the APP "END & SAVE WORKOUT" to store
the workout data.

The Xterra + App also allows you to sync your workout data with one of many fitness cloud
sites we support: Strava, Map My Fitness or Fitbit, with more to come. Syncing the App with
...
3. In the App click the Bluetooth icon in the top right corner to search for your Xterra +
   equipment.

4. Under the Bluetooth scan result list, select the machine for connect. When the App and
   equipment are synced, the Bluetooth® icon on the equipment’s console display will light up.
   Click “DISPLAY”,You may now start using your Xterra + App.

5. When your exercise is complete pressed in the APP "END & SAVE WORKOUT" to store
   the workout data. and you will be prompted to sync your data with each available fitness
   cloud site. Please note you will have to download the applicable compatible fitness
   App, such as Strava, Map My Fitness or Fitbit, etc, in order for the icon to be active and
   available.

Note: Your device will need to be running on a minimum operating system of IOS 10.1 or
Android 6.0 for the Xterra + App to operate properly.
...
to wipe the area between the frame and 6 - 8” under the belt. This should be done once every
other month to extend belt and deck life. Use water only - no cleaners or abrasives. A mild
...
tension adjusting bolts clockwise a 1/4 turn each
and inspect for proper tension by walking on the
belt and making sure it is not slipping or hesitating
with each step. When an adjustment is made to the
belt tension, you must be sure to turn the bolts on
both sides evenly or the belt could start tracking to one side instead of running in the middle of
the deck.
...
A 5mm Allen wrench is provided for this adjustment. Make tracking adjustments on the left
...
is too close to the left side, then turn the bolt only
a 1/4 turn to the right (clockwise) and wait a few
minutes for the belt to adjust itself. Continue to
make 1/4 turns until the belt stabilizes in the center
of the running deck.
...
performed every 40 hours of use, or earlier if you notice that the deck is dry or dirty. Please
...
reset the display:
1. In standby mode, hold the PROGRAM key for 3 seconds to reset the message.
...
expense. This list includes common problems that may not be covered under the treadmill’s warranty.
 ...
 Problem                Meaning                  Solution/Cause
  ...
  Showing             lubrication under          Reference belt deck lubrication procedure on page 17.
  LUBE                walking deck               2. Hold PROGRAM button for 3 seconds to reset


  Console             Console shows E1           1. Does the motor run after “Start” button is pressed?
  Showing E1          8~10 seconds after         2. Connector could be loose.
                      pressing “Start”           3. Check the cable connecting the console and controller
                      button.                    for proper connection.
  ...
  Showing E0          Malfunction
                                                 Any connector damaged?
                                                 3. Is safety switch defective?


  Console             Over Current               1. Check for proper walking belt alignment. Make sure to
  Showing E2          Protection                 lubricate belt periodically to minimize resistance.
                      Treadmill over loaded,     2. Is there any bearing damage?
                      controller protection is   3. Has the motor overheated?
                      activated.


  Console             Improper Motor input       1. Motor is not connected properly or not connected to
  Showing E4          voltage.                   the controller.
                      Motor is not connected
                      properly.


  Console             Communication              1. Check for proper connection between the console
  Showing E5          disconnected between       and controller.
                      the console and
                      the controller or
                      communication error

  Console             Controller                 1. Controller component failure
  Showing E6          Malfunction



  Console             Abnormal Power             1. Check for proper voltage input for treadmill.
  Showing E7          Input
...
Effective August 24, 2022- TREADMILL LIMITED WARRANTY
...
3. Proper connection to a power supply of sufficient voltage, replacement of blown fuses, repair of loose connections or defects in
...
3. Damages caused by services performed by persons other than authorized XTERRA Fitness service companies, use of parts other
     than original XTERRA Fitness parts, or external causes such as alterations, modifications, abuse, misuse, accident, improper
...
7. Products that do not have a warranty registration on file at XTERRA Fitness, Inc. XTERRA Fitness reserves the right to request
...
e« An appliance should never be left unattended when plugged in. Unplug from outlet when not in use, and
before putting on or taking off parts.

¢ Do not operate under blanket or pillow. Excessive heating can occur and cause fire, electric shock, or injury to
persons.

e This exercise equipment is not intended for use by persons with reduced physical, sensory or mental
capabilities, or lack of experience and knowledge.

e Use this appliance only for its intended use as described in this manual. Do not use attachments not
recommended by the manufacturer.

e Never operate this appliance if it has a damaged cord or plug, if it is not working properly, if it has been
dropped or damaged, or dropped into water. Return the appliance to a service center for examination and
repair.

¢ Donot carry this appliance by supply cord or use cord as a handle.

* Keep the cord away from heated surfaces.

e Never operate the appliance with the air openings blocked. Keep the air openings free of lint, hair, and the
like.
...
e The appliance is intended for household use.
...
DONOT USEAN EXTENSION CORD UNLESSIT IS 14AWG OR BETTER, WITH ONLY ONE OUTLET ON THE END. THE
TREADMILL SHOULD BE THE ONLY APPLIANCE IN THE CIRCUIT IN WHICH IT IS CONNECTED. DO NOT ATTEMPT
TO DISABLE THE GROUNDED PLUG BY USING IMPROPER ADAPTERS, ORIN ANY WAY MODIFY THE CORD SET.A
SERIOUS SHOCK OR FIRE HAZARD MAY RESULT ALONG WITH COMPUTER MALFUNCTIONS.

A\\ warwine: This product can expose you to chemicals including Toluene and Acrylamide which are
known to the State of California to cause cancer and birth defects or other reproductive harm.
For more information go to www.P65Warnings.ca.gov
...
#79 O8 x O18 x 1.5T #87 5/16" x UNC18 x 1/2" #88 08 x O23 x 1.5T
Flat Washer Button Head Socket Bolt Curved Washer
(6 pcs) (12 pcs) (6 pcs)
...
Jenue| SJaUMO OGZYL ‘Parvasay sayBiy |Iy 22OZO WOOd'SSSUCUS8IX MMM
