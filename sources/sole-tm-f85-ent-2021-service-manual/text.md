...
-------------------------------------------Table of Contents-------------------------------------------
                        ...
                        8. Treadmill Error Messages / Troubleshooting
                            ...
                            8.11 Troubleshooting Procedure Matrix for Electronic Issues
...
12. Disassembling and assembling of Parts
  ...
  12.9 Incline Motor Replacement
                                        ...
                                        3                     Service Manual
Special Note on ST538 CE version:
...
that the power input is 110VAC for normal version versus 230VAC for CE version and an additional Filter Choke circuit is added for CE
version as shown in the circuit diagram on next page.
...
3. Electrical Configurations
             ...
             12                Service Manual
SAFETY KEY:
 To fit on the console that activates all function, if no safety key, the console cannot be controlled. The display will show “Please Replace the
 Safety Key".
...
TREADMILL MOTOR:
 This is a variable speed for DC motor. To control the 0 – 180 DC volts（To control 0-90 DC voltages on 120Vac electronic power system）on the
 main controller, it can to increase or decrease speed of running belt.
...
CONSOLE
 ...
 The console comprised of key controls and TFT 10.1” TFT TOUCH PANNEL display.
 Main controller Include power supply 、motor driver control circuit and incline control circuit.

TREADMILL MOTOR

 It’s a variable speed on 0-180 volt DC motor on 230V. (0-90 volts DC motor on 120Vac electronic power system)
...
INCLINE MOTOR
 ...
 The green wire is ground.
                                                                        ...
                                                                        14                                       Service Manual
4. ST538 Treadmill Product Operation
...
Window Display Mode
   ...
   READY Mode
      ...
      START button or TOPCH PANNEL "START WORKOUT" to start treadmill on Manual Mode.
   SLEEP Mode
...
Function Button In Main Mode
  READY MODE
     ...
     Touch panel button: Press “       ” button to change each function. MANUAL can set using time, Pre-set PROGRAM and Fitness tests can set
     using time and speed, Heart Rate Control 1~2 can set time, age and the value of heart rate. User Program 1~2 can set time, speed and incline.
     Touch FAST button: If user doesn’t enter a setting then this button is non-functional.
     Touch SLOW button: If user doesn’t enter a setting then this button is non- functional.
     Touch UP button: If user doesn’t enter a setting then this button is non- functional.
     Touch DOWN button: If user doesn’t enter a setting then this button is non- functional.
     SPEED RAPID button: 7 preset buttons for rapid speed: 2，3，4，5，7，9，12 MI (3/6/9/12/15/18/22KM)
     INCLINE RAPID button: 7preset buttons for rapid incline: 1，3，5，7，9，12，15
...
RUN MODE
  ...
  Touch FAST button: Press the button to increase your speed and each increase is 0.1kph (0.1mph). If button is pressed continuously then
  speed increases to MAX SPEED quickly.
  Touch SLOW button: Press the button to decrease your speed and each decrease is 0.1kph (0.1mph). If button is pressed continuously then
  speed decreases to MIN SPEED quickly.
  Touch UP button: Press the button to raise position and each increase is 1, the maximum incline position is 15.
  Touch DOWN button: Press the button to lower position and each decrease is 1, the minimum incline position is 0.
  SPEED RAPID button: Speed will set to 2，3，4，5，7，9，12MI (3/6/9/12/15/18/22KM) speed quickly .
  ...
  DISPLAY KEY：It can change the three training diagrammatic, from             to           then to this         .




                                                                  21                                                        Service Manual
5.ST538 Treadmill Unit Block Diagrams
...
Treadmill Configuration
                          ...
                          23   Service Manual
6. ST538 Treadmill Basic Connections and Wiring
...
6.2 Display Board PCB Component Locations
 ...
 PCB Board Top
                                        ...
                                        27   Service Manual
PCB Board Bottom
    For Android 6 Console




                            28   Service Manual
For Android 10 Console
...
7.3 Important Grounding Instructions
 ...
 - DANGER - Improper connection of the equipment-grounding conductor can result in a risk of electric shock. Check with a qualified electrician
   ...
   a metal screw.
                                                                           ...
                                                                           37                                                              Service Manual
8. ST538 Treadmill Error Messages /
         Troubleshooting
...
   Prepare：
                                                      ...
                                                      Picture                         Tool name
                                                                         ...
                                                                         39                                                 Service Manual
8.1 Error Message：Display appears「PLEASE REPLACE THE SAFETY KEY」on the Message Window
...
 Cause of E0
 ...
 But possibly main control wires or component of lower controller is broken. (Because lower controller sent (+12V) signal via S/W of main control wire to upper
 control board to form a safety switch loop.)
...
1. Engineering Mode
1. Press 10 times on “Settings” letter to enter engineering mode when you’re at the Settings page. There are 6 main functions list：Engineering Mode、
Diagnostics、Machine Information、Maintenance、Lube Setup、Key Test




                                                                        70                                                         Service Manual
2. Press Engineering Mode to enter Calibration




Hit the block to enter the value by digital keyboard, and press ENTER to save and close the keyboard. The display will appear the max./min. value if the
entered value exceeds the range. The English range: Max.(12.0)/Min.(0.5); the Metric range: Max.(22.0); Min.(1.0)
...
GR Mode: Grade Return，Default: ON，Setting：ON/OFF； When GR MODE-OFF is switched to ON if the incline doesn’t back to zero, switch it to ON
  then go back to Setting, the incline should back to zero.
1. When the setting is ON：
   1.1 When the treadmill is power on, the incline should be at zero.
   1.2 When the user pause the machine during workout, the incline should be at zero.
   1.3 When end of workout and the display at SUMMARY page, the incline should be at zero.
   1.4 When the display at HOME page, the incline should be at zero.
   1.5 When unplugging the SAFETY KEY, the incline shouldn’t return zero. When plugging the SAFETY KEY, the incline should be at zero.
2. When the setting is OFF：
   2.1 When the treadmill is power on, the incline shouldn’t be at zero.

                                                                           72                                                    Service Manual
   2.2 When the users pause the machine during workout, the incline should be at current level and shouldn’t be at zero.
   2.3 When end of workout and the display at SUMMARY page, the incline shouldn’t be at zero.
   2.4 Press START to begin, the incline should be at zero or at the current set level.
   2.5 When unplugging the SAFETY KEY, the incline shouldn’t be at basic level. When plugging back the SAFETY KEY, the incline shouldn’t be at
       basic level.

Beep Mode: Buzzer switch Default：ON
    This especially indicate the finger hitting sound of the touch panel, not indicate the actual keys on console keyboard. When it switched to ON, it will
appear the buzzer sound if the users hit the touch panel. When it switched to OFF, the buzzer sound will be closed.
Display Mode: About the sleeping mode function and display of SAFETY KEY function . Default: ON. When the setting is ON, the console won’t power
  down if the users remove the SAFETY KEY. When the setting is OFF, the console will power down if the users remove the SAFETY KEY, the display
  will turn off to have the console power down automatically after 30 minutes of inactivity. The user can press any key to wake up the console.
Child Lock: Child Lock. Default: OFF, and the Child Lock is ineffective. Setting range: ON/OFF. If the user set the Child Lock ON, please repower the
machine or re-plug the safety key to execute. The display will go to IDLE MODE DM and will show 【CONSOLE LOCKED】. The display will also show
【Press Start and Display to enable operation】 at the bottom. The user can press START & DISPLAY key within 2 seconds to unlock the Console to
enter IDLE MODE.
                                                                            ...
                                                                            73                                                           Service Manual
3. Lube Setup
3.1 It’s the function to remind the user to lube deck, default time is 90 hours. The upper right on the display will show the total using hour. Setting range:
90~200 hours.
3.2 Power on the machine and if the total using hour reach the setting hours, the display will show the message【LUBE DECK PRESS STOP TO RESET
LUBE MESSAGE】 within 3 seconds, then the message will close and go to IDLE MODE.




3.3 When the display go to IDLE MODE, the Lube message won’t appear again until next setting hours(default 90 hours or user defined hours). And the
total using hour and distance won’t be clear, they will be kept accumulating.
3.4 When the display show the Lube message, the user can press STOP key to clear the message. And the Lube message won’t appear again until next
setting hours(default 90 hours or user defined hours). And the total using hour and distance won’t be clear, they will be kept accumulating.
3.5 Press STOP key to clear the Lube message. Or enter Lube Setup to hit 【Zeroing】, then the display will show 【Are you sure the lube parameter
need to be reset?】. Select Yes to reset the total Using hour or not.
3.6 If the total using hour doesn’t reach setting hour and the user decides to reset the Using hour, the accumulated using hour will reset as zero.
                                                                              ...
                                                                              74                                                            Service Manual
4. Key Test
4.1 Hit Key Test and the display will show you the name of the key, please press the according key to do key test.
4.2 The display will show a message【All Done!】within 5 seconds when all the keys have been pressed to finish the test.
...
When press “START”, treadmill doesn’t start.       1 Motor M+ or M- wire isn’t connected into 1 Please check and plug again.
                                                   ...
                                                   3 Treadmill controller shut down and LED Turn off the ON/OFFswitch and turn on power again.
                                                                                              3
                                                      would be ON.
...
SWITCH can’t be used.                    CONSOLE not connected properly.                      2. Replace with new cable.
                                         2 The connector of SPEED CABLE and
...
FOLDING INSTRUCTIONS
 ...
 Useboth handrails to maneuver the unit to the desired position.
 ...
  TO FOLD THE TREADMILL
    ...
    deck until it is secured by the locking telescoping tube assembly in center
    back of base.
 ...
  TO UNFOLD THE TREADMILL
    ...
    on theunlocking lever and slowly lower the running deck to the floor. The deck will
...
10. General Maintenance
           ...
           81             Service Manual
10.1 Tread belt and Deck
Your treadmill uses a very high-efficient low-friction bed. Performance is maximized when the bed is kept as clean as possible. Use a soft,
damp cloth or paper towel to wipe the edge of the belt and the area between the belt edge and frame. Also reach as far as practical directly
under the belt edge. This should be done once a month to extend belt and bed life. Use water only - no cleaners or abrasives. A mild soap and
water solution alongwith a nylon scrub brush will clean the top of the textured belt. Allow the belt to dry before using.
...
General Cleaning - Dirt, dust, and pet hair can block air inlets and accumulate on the running belt. On a monthly basis: vacuum
underneath your treadmill to prevent buildup. Once a year, you should remove the black motor hood and vacuum out dirt that may
accumulate. UNPLUG POWERCORD BEFORE THIS TASK.

BELT ADJUSTMENTS:

Tread-belt Tension Adjustment - Adjustment must be made from the rear roller. The adjustment bolts are located at the end of the step rails
in theend caps, as noted in diagram below.




                                                                         82                                                               Service Manual
Tighten the rear roller bolts only enough to prevent slippage at the front roller. Turn both tread-belt tension adjustment bolts in increments of 1/4
turneach and inspect for proper tension by walking on the belt at a low speed, making sure the belt does not slip. Keep tensioning the bolts until
the beltstops slipping.
...
The performance of your treadmill is dependent on the frame running on a reasonably level surface. If the frame is not level, the front and back
rollercannot run parallel, and constant belt adjustment may be necessary.
...
A 10 mm Allen wrench is provided to adjust the rear roller. Make tracking adjustments from the left side only. Set belt speed at approximately 3
to 5kph.
...
stabilizes inthe center of the running deck.

The belt may require periodic tracking adjustment depending on use and walking/running

characteristics. Some users will affect tracking differently. Expect to make adjustments as required to center the tread-belt. Adjustments will
becomeless of a maintenance concern as the belt is used. Proper belt tracking is an owner responsibility common with all treadmills.
...
BELT/DECK LUBRICATION
   First, you want to clean between the belt and deck to remove any debris that may be trapped. Use a clean, non-fraying rag, t-shirt, or light
   towel. Halfway between the end of the treadmill and motor cover, shove the garment under the belt until you can grasp it on both sides of
   the belt. Dragthe garment the length of the entire belt 1-2 times. Remove the garment.



   Do not lubricate with anything other than approved lubricant. Your treadmill comes with one tube of “Lube” and extra tubes can be ordered
   directlyfrom or your authorized dealer. You may also use a Lube-n-Walk kit that can be purchased through both aforementioned sellers.



   Keeping the deck lubricated at the recommended intervals ensures the longest life possible for your treadmill. If the lubricant dries out, the
   frictionbetween the belt and deck rises and places undue stress on the drive motor, drive belt and electronic motor control board, which could
   result in catastrophic failure of these expensive components. Failure to lubricate the deck at regular intervals may void the warranty.
   ...
   dry. Itis recommended that you reach between the belt and deck to verify there is lubrication present, every other month.
...
3. Use 14mm T-shaped socket wrench to loosen 4 screws, use 14mm T-shaped socket wrench to loosen a side tension
 ...
 Reassemble in reverse order of disassembly, re-hook belt (readjust belt to be parallel with main frame after re-hooking). Do
 not tighten 4 securing screws yet.
...
2. Use M6 L Allen wrench to loosen 2 screws on the rear roller.
                                                             ...
                                                             96                    Service Manual
3. Use Phillips head Screwdriver to remove screws. Disassemble left and right upright bracket cover. Remove the motor cover.
Use 14mm T-shaped socket wrench to loosen 4 securing screws. Use 14mm wrench to loosen a side tension screw as well as a
rear drive belt tension screw in order to loosening the drive belt.
...
4. Use 14mm open end wrench to adjust belt tension using tension measuring device. Adjust to white LBS area between
70-75LBS. Use 14mm T-shaped socket wrench to secure and tighten 4 securing screws. Connect grounding wires and motor
wires (red M+, black M-)
                                                            ...
                                                            97                                                 Service Manual
12-7     Running Deck/ Belt & Cushion Replacement (※Be sure to disassemble the cylinder

     prior to this replacement procedure)
1. After folding the treadmill, use M5L Allen wrench and 13mm wrench to release the screw which secure the cylinder and take it
apart. Perform item (F), procedures to replace front and rear rollers.




2. Remove the left and right foot rail cap. Use a Phillips head screwdriver to remove 6 screws and bracket from back of foot rail.
Remove the foot rail in the direction of the indicating arrows.
...
12-8   Speed Sensor Replacement(including the wire)
 ...
 Remove speed sensor wiring and proceed with parts replacement. After replacement, a test to check if the sensor registers the
 magnet is required.
...
12-9 Incline Motor Replacement
 ...
 2. Incline Range must be adjusted to 225mm minimum prior to installation
                                                             ...
                                                             101                   Service Manual
3. Use M8 L wrench and # 14 mm wrench to assemble new incline motor.
