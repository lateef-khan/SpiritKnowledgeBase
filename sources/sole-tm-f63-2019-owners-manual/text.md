...
If voltage varies by ten percent (10%) or more, the performance of your treadmill may be af- fected.
...
ing provides a path of least resistance for electric current, reducing the risk of electric shock. This
...
4. To stop the tread-belt, press Stop key or ae away Safety Key.
...
<!-- render-vs-extraction: 173 words the text layer does not have; tesseract --psm 4 at 300 dpi -->
...
dots - each 8 units high - indicate each segment of a workout
for the . The dots are only to show an approximate level of effort
in speed or incline. They do not necessarily indicate a specific
value - only an approximate percent to compare levels of
intensity. In operation, the speed/incline dot matrix window will
build a profile “picture” as values are changed during a workout.
When the word “Speed” is lit above the dot matrix, it indicates
that the Speed profile is being displayed. When the word
...
1. Inidle mode, the heart rate Bluetooth LED on the keypad will not be lit if the Bluetooth
...
to pypass this and go directly to the ee of the program, press Start again.

fF HH G ANN jer
Cnt i— CLI pm
Ti — fin m—— pean .
WW Wee 3] | me 8
Wwenis ig 200] | 97. pees oP) 50
ME vee COU a
it Shea =
...
1. Select User 1 or User 2 via the Program 4/¥ keys then Hlen ( / JIEEO
WOCN § 7 USER

press Enter. Note that the dot matrix display portion
will have a single row of dots at the bottom (Unless
there is a previously stored program). If there is a (yr asst
program stored under the button that is pressed, it will tt wy ;
...
it, press the any A key, then “B” will be displayed; if the
any V key is pressed, the letter “Z” will be displayed.
After selecting the appropriate letter, press Enter. The

letter “A” will again be displayed and blinking. Repeat ~~, ww in Mame

the procedure until all letters of your first name are | . | | er uu | am

programmed (7 characters maximum). When your I: Il | | oo

name is displayed, press Stop and it will be stored

under either User 1 or User 2. ‘ii rif || seareneenrersneee
2. Next the Message Window is flashing Age. Use the | a= .

Incline 4/¥ keys to adjust. Press Enter. This is a must WU UU Ca,
