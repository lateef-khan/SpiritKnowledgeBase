...
mH = This appliance is not intended for use by persons with reduced physical, sensory or
...
1. Remove the treadmill from the carton and position it aside on a smooth, level
...
2. Slide the lower handlebar covers (63 & 65) onto the console masts (notice
orientation).

3. Slide the Frame Base Caps (161 L & 162 R) over the Upright tubes (4 & 5).
Connect the Lower Computer Cable (51) to the Middle Computer Cable (50)
...
1. Attach the Upright Tubes (4 & 5) onto the Frame Base (2) with eight Button
...
Upper (49). Tuck the excess cable into the hand rail tubing to prevent it from
getting pinched.
...
1. Attach the Console Assembly (52) onto the Upright Tubes (4 & 5) and
attach with four Button Head Socket Bolts (133) and four Split Washers (113).
Tighten using the M6 Allen Wrench (132). Tighten completely.

ASSEM BLY STEP 6 HARDWARE STEP 6

#168. 3.5x 16mm
Sheet Metal
Screws (6 pcs)

1. Attach the Left (63) and Right (65) Lower Handlebar Covers to the console
with six Sheet Metal Screws (168). Tighten with the Combination M5 Allen
Wrench & Phillips Head Screw Driver (131).
...
1. Attach the Support tube (15) between the upright tubes (4 & 5) with four
Button Head Socket Bolts (151). Tighten bolts firmly.

2. Place the Beverage Holder (150) on top of the Support tube (15) as shown in
the illustration.

3. Place the magnet of the safety key (75) in between the Start and Stop
buttons (if itisn’t already attached). The treadmill will not function without
this in place.
...
1. Finish tightening the button head bolts (from Step 2 above) that secure the
console masts to the frame.

2. Attach the Console mast covers (161 & 162) to the Frame Base (2) with six
Tapping Screws (120). Tighten screws using the Combination M5 Allen
Wrench & Phillips Head Screw Driver (131).
...
CD, OR SMARTPHONE) HEADPHONE JACK
...
Total time will show how many hours the treadmill has been in use and the Total distance
will show how many miles (or Kilometers if the treadmill is set to metric readings) the treadmill
has gone. The treadmill will then enter idle mode, which is the starting point for operation.
...
, Press the Start key to begin belt movement then adjust to the desired speed using the
...
and 15 on the left side of the console (incline).

B= The Incline Window display will indicate incline position as adjustments are made.
...
Eighteen columns of dots, 8 high for the F85 indicate

each segment of a workout. The dots are only to show an
...
matrix it indicates that the speed profile is being SPEED INCLINE
displayed and when “INCLINE” is lit the incline profile is
being displayed.
...
The F85 have ten built-in programs: one Manual program, five preset profiles, two user defined
...
increase in workload to a high level and forces you to sustain it. This is designed to strengthen and tone your legs
...
1. Select User 1 or User 2 via the Program keys.
Note that the dot matrix display portion will have a single row Meco oto yy iero oo
of dots at the bottom (Unless there is a previously stored WICK § / USER C
program).

2. If there is a program stored under the button that is pressed,
it will be retrieved. If not, you have the option of
programming in your first name. The name message window
will display the letter “A”. To change it, press the any @ key,
then “B” will be displayed; if the any WV key is pressed, the
letter “Z” will be displayed. After selecting the appropriate

letter, press STOP. The letter “A” will again be displayed.
Repeat the procedure until all letters of your first name are
programmed (7 characters maximum). When your name is
displayed, press ENTER and it will be stored under either User
1 or User 2.

3. Next the menu will be display Age window first. Use the
Incline A/V keys to adjust. Press Enter. This is a must to
continue even if age is not adjusted.

4. The Weight window will now be display a body Weight value.
Enter your body weight and press Enter.

5. The Time Window will now be display a Time value. Adjust
the Time and press Enter.

6. The first column (segment) will now be blinking. Using the
Speed 4/V or keys, adjust the speed level to your desired
effort for the first segment then press Enter. The second
column will now be blinking. Repeat the above process until
all segments have been programmed. The first column will be
blinking again. This is for the incline programming. Repeat the
above process to program all segments for incline.
...
8. The profile picture will be re-scaled to fit in the window, but the actual speed and incline
...
number by pressing the Speed A/V keys (console or handrails) and press Enter to continue.
...
the screen displays Engineering Mode Menu.

2. Press the Speed A button (or Speed V button to go backwards) until “LUBE
MESSAGE RESET”; press Enter to reset lube message.

3. Press Stop to exit Engineering mode and resume use of your treadmill.
...
Continue to hold the Start and Speed A key until the screen displays “CALIBRATION MODE”.

3. You will now be able to set the display to show Metric or English settings (Miles vs.
Kilometers). To do this, press ENTER key to show which you want.
...
2. You can now scroll through the menu using the Speed 4/¥ keys. Use the Stop key to
...
F. | CHILDLOCK - Allows you to lock the keypad so no unauthorized use of the machine is allowed. When
the child lock is enabled, the console will not allow the keypad to operate unless you press and hold the
...
<!-- render-vs-extraction: 339 words the text layer does not have; tesseract --psm 4 at 300 dpi -->
